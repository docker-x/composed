...
// parseSetValues parses "key=val" and "a.b.c=val" strings into nested maps.
...
// parseEnvValues parses "KEY=VAL" pairs into a flat string map.
...
// deriveComponentName extracts a short service name from a source reference
// (OCI URL, image reference, local path, or plain name).
func deriveComponentName(source string) string {
	s := strings.TrimPrefix(source, "oci://")

	// Strip tag (after last colon, if not part of the registry like localhost:5000)
	...
	// Local paths: use directory name for files, last segment for directories
	...
	// Anything with slashes: take the last path segment
	if i := strings.LastIndex(s, "/"); i >= 0 {
		return s[i+1:]
	}
